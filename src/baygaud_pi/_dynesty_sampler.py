...
def enforce_min_nlive(ndim: int, _params: dict, verbose: bool = True):
    """
    dynesty의 권장 최소 nlive (= 1 + ndim*(ndim+1)//2)를 보장.
    common_kwargs를 제자리(in-place)에서 수정.
    """
...
def dynamic_baygaud_nested_sampling(num_cpus_nested_sampling, _params):
    ...
    # YAML -> 환경 변수 dict (threading 섹션/토플레벨 키 모두 지원)
    ...
    def baygaud_nested_sampling(
        ...
        for j in range(0, _je - _js):
            ...
            # --- (1) Matched-filter seed 검색: YAML 파라미터 사용 ---
            ...
            # --- (3) 단일 가우시안 경계: YAML 파라미터 사용 (normalised units) ---
...
#                    )
            ...
            # Before trying to the first single Gaussian function fit process
            # check peak_sn 
            ...
            for k in range(0, _max_ngauss):
                ...
                # set minimum nlive given common_kwargs[nlive]
                ...
                if ngauss == 1: # check the peak s/n
                    ...
                    _sigma_sgfit = _gfit_results_temp[3] * (v_max_phys - v_min_phys) # sigma
