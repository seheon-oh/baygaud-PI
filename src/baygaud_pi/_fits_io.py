...
def read_datacube(_params):
    ...
    with fits.open(_params['wdir'] + '/' + _params['input_datacube'], 'update') as hdu:
        ...
        try:
            hdu[0].header['RESTFREQ'] = hdu[0].header['FREQ0']  # GIPSY 처리 FITS 대응
        except:
            pass
...
#    print("")
    ...
    # ── 요약 딕셔너리 구성 (print_cube_summary에서 바로 사용 가능)
    info = {
        "naxis1": _naxis1,
        "naxis2": _naxis2,
        "naxis3": _naxis3,
...
def moment_analysis_alternate(_params):
    ...
    # 1) CDELT3가 m/s인 경우(예: -2000 m/s), Hanning 0회
    info = min_sigma_from_cdelt3(-2000.0, unit="m/s", hanning_passes=0)
    # dv_kms=2.0 → sigma_min≈2/√12≈0.577 km/s, fwhm_min≈1.36 km/s

    # 2) CDELT3가 km/s인 경우(예: 2 km/s), Hanning 1회
...
def min_sigma_from_cdelt3(
    ...
    """
    관측 스펙트럼에 '가우스를 컨볼브 없이' 바로 피팅할 때,
    물리적으로 의미 있는 최소 가우스 σ를 계산합니다 (km/s).

    σ_min^2 = (Δv^2 / 12) + n * (Δv^2 / 2)
      - Δv: 채널폭 (km/s)
      - n : Hanning smoothing 횟수 (0,1,2,...)
    반환값에는 등가 FWHM도 함께 포함합니다.

    Parameters
    ----------
    cdelt3 : float
        스펙트럼 축 증가량(CDELT3). 부호는 무시됩니다.
        단위는 `unit` 인자로 지정(m/s 또는 km/s).
    unit : {"m/s", "km/s"}, default "m/s"
        cdelt3의 단위.
    hanning_passes : int, default 0
        적용된 Hanning smoothing 횟수(0, 1, 2, ...).

    Returns
    -------
    dict
      - dv_kms           : 채널폭 Δv (km/s)
      - sigma_min_kms    : 최소 허용 σ (km/s)
      - fwhm_min_kms     : 2.35482 * σ (km/s) (등가 가우스 FWHM)
      - var_box_kms2     : Δv^2 / 12
      - var_hanning_kms2 : n * (Δv^2 / 2)

    Notes
    -----
    * 이 σ_min은 **LSF(채널 적분+Hanning)를 모델에 넣지 않고** 바로 가우스를 맞출 때
      **물리적 하한(bound)** 으로 쓰는 값입니다.
    * LSF와 **컨볼브해서** 맞출 경우에는 모델에 LSF를 포함하고
      내재 σ는 거의 0에 가까운 바닥만 두면 됩니다.
    """
    # 단위 정규화 + 절대값
